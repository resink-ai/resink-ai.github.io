...
- **ABI Option A (static linking, not dlopen) — step 2 closed; hot-swap test pending loop+1 (2026-06-20).** Originally carried since loop 2026-05-16. **Step 2 closed at loop 2026-06-13:** the in-tree `crates/nanofab-plugin-dim-user/` workspace member (a manual slot-fill of AE's `codegen-scd2-node/templates/scd2_maintainer/lib.rs.tmpl` against the `dim_user` schema) builds as a `cdylib` at `target/release/libnanofab_plugin_dim_user.{so,dylib}` and exports the three C-ABI symbols (`nanofab_node_new` / `nanofab_node_process` / `nanofab_node_drop`) per AE's template. The supervisor's `--features dlopen-plugins` path opens the cdylib via `libloading::Library::new(...)`, resolves the symbols, and wraps them in the safe `PluginNode` handle in `crates/nanofab-supervisor/src/plugin_loader.rs`. The integration test `crates/nanofab-supervisor/tests/dlopen_integration.rs` exercises the FFI roundtrip end-to-end on a known `dim_user` insert event, asserting `NANOFAB_NODE_OK = 0`. **Canonical going-forward invocation:** the `--features dlopen-plugins` Cargo feature flag (see `docs/user-guide.md` § "Plugin loading modes" for the build commands and the `make mvp-loop-dlopen` Make target). Static linking remains as a rollback fallback under `--no-default-features --features static-plugins`. **Step 3 (hot-swap correctness test) slipped to loop+2** (was loop+1) — joint AE + resink-core deliverable per ADR-2026-05-16-001's existing schedule, slip-by-one recorded at loop 2026-05-12-0645 in this loop's exec summary; the slip reason was "deployment focus this loop crowds out the hot-swap test." Per ADR-2026-05-30-002's loop+N convention, the ADR body grandfathers under absolute-date authoring; this slip is plan-side bookkeeping only. **Where in code:** `crates/nanofab-supervisor/Cargo.toml` (`[features]` block + `libloading = { version = "0.8", optional = true }`); `crates/nanofab-supervisor/src/plugin_loader.rs` (the `PluginNode` wrapper); `crates/nanofab-plugin-dim-user/` (the in-tree slot-filled artifact); `crates/nanofab-supervisor/tests/dlopen_integration.rs` (the FFI roundtrip gate); `synthetic_tenants/closed_loop_v0/Makefile` (the `mvp-loop-dlopen` target).
