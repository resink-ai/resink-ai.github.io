...
- KR2.2: ⚠️ **Deferred.** Branch protection on master cannot be enabled — the resink-core repo is private on a free GitHub plan, which doesn't permit branch-protection rules or rulesets on private repos (paid feature). API endpoints return `403` with an upgrade prompt. **The repo stays private** (visibility decision is reserved per memory `feedback-no-public-repo.md`); the unblocking path is a paid-plan upgrade. Until then the helm CI runs but isn't a required gate; the operator manually verifies green-CI in the PR UI before merging.
