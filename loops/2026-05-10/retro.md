...
<!-- original-frontmatter:
  ...
  status: active
  loop: 2026-05-10
  links: parent: board/exec-summaries/2026-05-10.md
-->
...
- **Owner:** board.
- **Timing:** **deferred to loop 2026-05-16.** Same-loop bandwidth conflicts with AE's Bundle B (rituals editing); stacking two retro-class ritual changes on the same files would multiply merge friction. P1 picks up in the loop after Bundle B lands.
...
- **Timing:** **deferred to loop 2026-05-23.** Pairs naturally with ADR-005 (carryover-load section in brief) and ADR-006 (out-of-retro routing path) — all three are small conventions/ritual edits and land more cleanly batched.
...
- **Timing:** **picked up loop 2026-05-16** (brief authoring step).
...
- **Proposed change:** A future CEO brief (loop 2026-05-16 or 2026-05-23) includes an objective to charter a Product UX team. Use `org-os/playbooks/onboard-application-team.md` for the application-team standup. The team's first product-shaped OKR follows the same pattern resink-core and sim-farm did when they joined.
...
- **ADR-003 (P1)** "Verify state-claims at ritual transitions." Draft placeholder at [`board/decisions/2026-05-10-003-verify-state-claims-at-ritual-transitions.md`](../decisions/2026-05-10-003-verify-state-claims-at-ritual-transitions.md). Status: `draft`. Owner: board. Picked up loop 2026-05-16.
- **ADR-004 (P2)** "Contract (or unified hand-off-document) artifact type." Draft placeholder at [`board/decisions/2026-05-10-004-contract-artifact-type.md`](../decisions/2026-05-10-004-contract-artifact-type.md). Status: `draft`. Owner: board. Picked up loop 2026-05-23.
...
- **ADR-005 (P1 from 2026-05-09 retro):** "Carryover load tally added to CEO brief authoring step." Draft. Picked up loop 2026-05-23.
- **ADR-006 (P2 from 2026-05-09 retro):** "Out-of-retro routing path for org-os change proposals." Draft. Picked up loop 2026-05-23.

ADR-005 + ADR-006 + this retro's ADR-004 batch cleanly at loop 2026-05-23 (three conventions/ritual edits). This loop's ADR-003 lands earlier (2026-05-16) because it is the highest-priority retro item — it directly addresses the gate-slip pattern that bit AE this loop.
