...
- **Approve Docker Desktop on the developer machine (DevOps standing-answer carry from loop 2026-05-30 § Asks).** Install Docker Desktop on the IC's Mac; run minikube with `--driver=docker`. This is one tool to install (Docker Desktop bundles the daemon) rather than two (separate daemon + minikube install). The `minikube-smoke` Helm target was authored at 2026-05-23 and unrun at 2026-05-30; this loop runs it.
...
- KR1.4: Marketplace tests pass: `bash repos/resink-ai/resink-marketplace/tests/run-all.sh` exits 0 with no regressions vs the 98 pass / 0 fail / 3 skip baseline from loop 2026-05-30.
...
Why it matters: This is retro P1 from loop 2026-05-23 — the highest-priority architectural-debt item. Engine 0.2.0 hard-codes `KEY_COLUMNS = ("user_id", "valid_from")` and payload columns `(email, country, valid_to, is_current)`, which forced resink-core to use an isomorphic column-shape compromise for `dim_account`. The multi-dim widening at 2026-05-23 was shape-only; semantic multi-table support requires this lift. Closing it (a) restores semantic correctness to resink-core's `dim_account`, (b) closes a third-loop carry, (c) unblocks any future third dim with a different PK name.
...
- **ADR-2026-05-30-001 (verify predicted syntax in OKRs)** — deferred to loop 2026-06-13 per 2026-05-30 retro P2 timing.
- **`org-os` conventions enum extension to admit `action`** — deferred to loop 2026-06-13; same-class batch with P2.
- **P3 (documentation-loop playbook extension)** — deferred to loop 2026-06-13 or later per 2026-05-30 retro timing; requires a second documentation-loop occurrence for the two-loop track record.
- **Real Kafka, real KV, multi-tenant deployment** — all out of scope unchanged.
- **Frontmatter-lint CI script** — fourth consecutive defer per loop 2026-05-08 ADR-001 framing.
