...
- **`architecture.md` § "System shape"** correctly summarizes the verdict layer ("Engine version is 0.2.0; mode is A (batch). The pytest suite is 9 tests, all passing as of loop 2026-05-23") and gates the `make verdict` step (step 7) on `overall_pass = true`.
...
- **Schema-aware join + payload columns** (P1 retro item from 2026-05-23) — engine 0.3 extension to read schema-driven key + payload columns; interfaces with DE (canonical per-dim schema-JSON shape) and resink-core (orchestrator passes the schema reference). Carries to loop 2026-06-06.
